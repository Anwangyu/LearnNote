...
public interface ExpenseDao {
    ...
    /**
     * 分页查询所有的指定empId的expense信息
     *
     * @param empId    需要查询的empId
     * @param startRow 分页时候开始的记录位置
     * @param endRow   分页时候结束的记录位置
     * @return 返回存储所有指定查询的List容器
     * @throws SQLException 持久层产生的sql异常
     */
    List<Expense> listExpensesByEmpId(String empId, int startRow, int endRow) throws SQLException;
     ...
     * 查询指定条件下有多少条报销单
     *
     * @param empId 特定的empId
     * @return 返回指定的用户有多少条报销单
     * @throws SQLException sql 异常
     */
    int getExpenseSumBy(String empId) throws SQLException;
}
